...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:LucasWeslley\n' // Nama kamu
            + 'ORG:BOT;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=559188170473:+55 9188170473\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '/','.','*'
blocked = []
limitawal = '999999999'
...
async function starts() {
	...
	client.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color(' Escaneie o código para se conectar ao bot'))
	})
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD LUCASBOT*',
					benned: 'Você para a banda, por favor, contate o proprietário para abrir sua banda',
					ownerG: '[❗] Lucas? Este é um recurso especial para o Lucas ❌',
					ownerB: '[❗] Lucas? Este é um recurso especial para o Lucas ❌',
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmão. Você não está registrado. Registre-se para fazer amizade com o LucasBot por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar Lucas|19\n\n • Lucas Bot •`,
			...
			const ownerNumber = ["559188170473@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"559188170473@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["559188170473@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["559188170473@s.whatsapp.net"]//ubah nomor lo
			const premium = ["559188170473@s.whatsapp.net","558296156440@s.whatsapp.net"] 
...
buf = fs.readFileSync(`./src/Nezuko.mp3`)
client.sendMessage(from, buf, audio, {mimetype: 'audio/mp4', quoted: mek, ptt: true})
}
if ((budy === 'BLOOM') || (budy === 'Bloom') || (budy === 'bloomi')) {
...
if ((budy === 'oi boti') || (budy === 'Oi bot') || (budy === 'OI BOT')) {
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
	...
	if (messagesC.includes("fdp")){
		...
		if (messagesC.includes("bot")){
			...
			if (messagesC.includes("bdah")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/bahc.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
	
				if (messagesC.includes("Lucasgs")){
			client.updatePresence(from, Presence.composing)
			...
			if (isCmd && isGroup) console.log('\x1b[1;31m~\x1b[1;37m>', '[\x1b[1;32mEXEC\x1b[1;37m]', time, color(command), 'DE', color(sender.split('@')[0]), 'in', color(groupName), 'args :', color(args.length))
			if (!isCmd && isGroup) console.log('\x1b[1;31m~\x1b[1;37m>', '[\x1b[1;31mRECV\x1b[1;37m]', time, color('Message'), 'DE', color(sender.split('@')[0]), 'in', color(groupName), 'args :', color(args.length))
			switch(command) {
				case 'help':
				case 'menu':
...
exec(`webpmux -set exif ${addMetadata('LucasBOT', 'Lucas')} ${rano} -o ${rano}`, async (error) => {
fs.unlinkSync(media)
reply(ptbr.stick())
})
})
...
.on('error', function (err) {
console.log(`Error : ${err}`)
exec(`webpmux -set exif ${addMetadata('Zap-BOT', 'Lucas')} ${rano} -o ${rano}`, async (error) => {
fs.unlinkSync(media)
    ...
    break
				...
				case 'bloquear':
				 ...
				 client.chatRead (from)
					...
					client.sendMessage(from, `Certo, bloqueado. ${body.slice(7)}@c.us`, text)
					break
				case 'premiumlist':
					teks = '╭────*「 *PREMIUM USER👑* 」\n'
            ...
            case 'smule':
				...
				case 'ttp':
                  ...
                  case 'desbloquear':
					...
					client.sendMessage(from, `Ok, Desbloqueado. ${body.slice(9)}@c.us`, text)
				    break
                case 'image':
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOS Lucas BOT 👑* 」\n\`\`\``
					no = 0
...
// recursos anime, random @Lucas domina vida
...
if (args.length < 2) return client.sendMessage(from, 'Digite o texto', text, {quoted: mek})
dtt = body.slice(8)
ranm = getRandom('.mp3')
dtt.length > 800
...
break
				...
				case 'walpaperanime':
					...
					break
					case 'dado':    
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o Lucas ou digite *.Daftarvip* para adquirir o acesso Premium!' ,text, { quoted: mek })
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT Lucas ✨* 」\n*│+ wa.me/5547992091566*╰──────*「 *Lucas* 」*────*\n\n*_SE QUER SER ADMIN DO BOT Lucas_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				if (!isUser) return reply(mess.only.userB)
					...
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+ wa.me/554792091566/*╰──────*「 * Lucas* 」*────*\n\n*_SE QUER SER UM USUÁRIO PREMIUM DO Lucas BOT_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
           ...
           case 'shitlist':
               ...
               break
				...
				case 'dono':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.ibb.co/hx5WqZw/Polish-20210606-085548537.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* LucasWeslley\n*WPP:* wa.me/+559188170473\n*INSTA:* @LucasWeslley1\n\n\nEspero que tenham gostado do bot :D ️'})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
        ...
        case 'promover':
				...
				case 'banir':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply(' marque o alvo que você quer banir!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
